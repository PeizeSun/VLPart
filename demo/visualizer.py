...
def _create_text_labels(classes, scores, class_names, is_crowd=None, args=None):
    ...
    supervision = ''
    if args is not None:
        valid_types = ['fully_supervised', 'unsupervised', 'open_world', 
                       'large_vocabulary', 'open_vocabulary', 'recall']
        supervision = args.MODEL.SUPERVISION
        assert supervision in valid_types
    labels = None
    if classes is not None:
        if supervision == 'unsupervised' or supervision == 'open_world' \
                or args.MODEL.EVAL_PROPOSAL:
            labels = ['' for i in classes]

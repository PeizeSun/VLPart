...
from .cocoeval_openworld import COCOEvalXclassWrapper
from detectron2.data.datasets.builtin_meta import COCO_CATEGORIES
from ..data.meta_obj365 import cat_obj365


# find pascal_classes in coco_classes
...
pascal_ids_in_coco = [coco_names[name_cat] for name_cat in pascal_classes]


# find coco_classes in obj365_classes
coco_meta = MetadataCatalog.get("coco_2017_train")
coco_classes = [name.title() for name in coco_meta.thing_classes]
...
coco_ids_in_obj365 = [obj365_names[name_cat] for name_cat in coco_classes]


class COCOOpenWorldEvaluator(DatasetEvaluator):
    """
    COCOEvaluator on open-world setting.
    Supported datasets: 
        COCO(VOC)-COCO(Non-VOC), 
        COCO-UVO, 
        COCO-Objects365
    
    To evaluate on unknown categories, optional model includes class-agnostic and class-aware
    Known categories are optionally included in the evaluation result. 
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        self._logger = logging.getLogger('detectron2.clusterdet.evaluation.coco_openworld_evaluation')
        ...
        if tasks is not None and isinstance(tasks, CfgNode):
            ...
            tasks=("bbox", "segm") if cfg.MODEL.MASK_ON and cfg.MODEL.MASK_EVAL_ON and 'obj365' not in dataset_name else ("bbox", )
            self._tasks = tasks
        ...
        self.agnostic_mode  = cfg.OPENWORLD.EVALUATE_CLASS_AGNOSTIC
        self.classwise_mode = cfg.OPENWORLD.EVALUATE_CLASS_AWARE
        self.exclude_known  = cfg.OPENWORLD.EVALUATE_EXCLUDE_KNOWN
        if self.exclude_known:
            assert 'coco' in dataset_name or 'obj365' in dataset_name
    ...
    def _eval_predictions(self, predictions, img_ids=None,
        ...
        for task in sorted(tasks):
            ## Class agnostic evaluation is used to compute AP and AR.
            ## Class wise evaluation is used to compute AR.
            coco_eval = (
                _evaluate_predictions_on_coco(
                    ...
                    # eval_obj365=eval_obj365,
                    agnostic_mode=True,
                    classwise_mode=self.classwise_mode,
                    exclude_known=self.exclude_known,
            ...
            eval_mode='class_agnostic' if self.agnostic_mode and not self.classwise_mode else 'class_wise'
            res = self._derive_coco_results(
                coco_eval, task,
                class_names=self._metadata.get("thing_classes") if not self.agnostic_mode or self.classwise_mode else None,
                eval_mode=eval_mode
            )
            self._results[task+'_'+eval_mode] = res
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None, eval_mode='class_agnostic'):
        """
        ...
        results = {
            ...
            for idx, metric in enumerate(metrics) if ('AP' in metric and eval_mode == 'class_agnostic') or 'AR' in metric
            }
...
'''
using the instances_to_coco_json of SparseInst
'''
...
def _evaluate_predictions_on_coco(
    ...
    if agnostic_mode and exclude_known:
        assert 'coco' in dataset_name or 'obj365' in dataset_name
        ids_known = coco_ids_in_obj365 if 'obj365' in dataset_name else pascal_ids_in_coco
    ...
    # if not (agnostic_mode and ignore_known):
    if not agnostic_mode:
        coco_eval = (COCOeval_opt if use_fast_impl else COCOeval)(coco_gt, coco_dt, iou_type)
    else:
        coco_eval = COCOEvalXclassWrapper(coco_gt, coco_dt, iou_type)
    
    # maxDets should be greater enough than k of AR@k evaluation
    # because the cross-dataset AR evaluation does not count those
    # predictions on the 'seen' classes into the budget (k), to avoid
    # evaluating recall on seen-class objects.
    coco_eval.params.maxDets = (10, 20, 30, 50, 100, 300, 500, 1000, 1500)

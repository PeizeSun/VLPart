...
class ParserNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model
        self.model = ParserNet.patch_vit_resolution(self.model, stride=stride)
    ...
    def from_config(cls, cfg):
        model_type = cfg.DINO.MODEL_TYPE
        ret = {
            'model_type': model_type,
            'model': ParserNet.create_model(model_type),
            'stride': cfg.DINO.STRIDE,
            'layer': cfg.DINO.LAYER,
            'facet': cfg.DINO.FACET,
    ...
    def patch_vit_resolution(model: nn.Module, stride: int) -> nn.Module:
        ...
        model.interpolate_pos_encoding = types.MethodType(
            ParserNet._fix_pos_enc(patch_size, stride), model)
        return model
    ...
    def _register_hooks_multi(self, layer: int, facets: List[str]) -> None:
        """
        register hook to extract features.
        :param layers: layers from which to extract features.
        :param facet: facet to extract. One of the following options: ['key' | 'query' | 'value' | 'token' | 'attn']
    ...
    def extract_descriptors_multi(self, batch: torch.Tensor, layer: int = 11, facets: List[str] = ['key'],
                            bin: bool = False, include_cls: bool = False) -> torch.Tensor:
        for facet in facets:
    ...
    def basedata_extract_save(self, batched_inputs, images):
        descriptors = self.extract_descriptors(images.tensor, self.layer, ["token"], include_cls=True)
    ...
    def find_nearest(self, images):
        descriptors, = self.extract_descriptors(
            images.tensor, self.layer, ['token', 'attn'], include_cls=True)
        token = descriptors[0][0][0]
        sim = nn.CosineSimilarity(dim=1, eps=1e-6)(self.basedata, token[None])
        nearest_idx = sim.argmax()
        return self.basedata_id2filename[nearest_idx.item()]
    ...
    def load_base_image(self, filename):
        ...
        pil_image = transforms.Resize(224, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        assert 'ParserNet only support inference'
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]],
        ...
        key_filename = self.find_nearest(images)

        key_image_tensor, key_image = self.load_base_image(key_filename)
        descriptors1 = self.extract_descriptors(key_image_tensor, self.layer, self.facet)
        num_patches1, load_size1 = self.num_patches, self.load_size

        descriptors2 = self.extract_descriptors(images.tensor, self.layer, self.facet)
        num_patches2, load_size2 = self.num_patches, self.load_size

        # extracting saliency maps for each image
        saliency_map1 = self.extract_saliency_maps(key_image_tensor)[0]
        saliency_map2 = self.extract_saliency_maps(images.tensor)[0]
        # saliency_map1 = saliency_map2

        # threshold saliency maps to get fg / bg masks
        fg_mask1 = saliency_map1 > self.thresh
        fg_mask2 = saliency_map2 > self.thresh
        ...
        bbs_mask = torch.bitwise_and(fg_mask1, fg_mask2_mask_new_coors)
        ...
        if self.vis_period == 1:
            return self.draw_correspondences(
                batched_inputs, key_image,
                img1_indices_to_show, img2_indices_to_show,
                num_patches1, num_patches2
            )

        key_image_id = key_filename.split('_')[0] + key_filename.split('_')[1]
        ...
        for instance_id in resize_key_part_map:
            ...
            mask1 = torch.tensor(resize_key_part_map[instance_id].reshape(-1))
            ...
            bbox = mask_to_box(resize_mask2)[0].tolist()
            mask_ = resize_mask2[0].cpu().numpy().astype(np.bool_)
    ...
    def draw_correspondences(self, batched_inputs, key_image,
                             img1_indices_to_show, img2_indices_to_show,

...
class ParserNet(nn.Module):
    ...
    def basedata_extract_save(self, batched_inputs, images):
        descriptors = self.extract_descriptors(images.tensor, self.layer, ["token"], include_cls=True)[0]
    ...
    def load_base_image(self, filename):
        ...
        pil_image = transforms.Resize(self.min_size, interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]],
        ...
        # bbs_mask = (nn_2[nn_1] == image_idxs) > -1  # disable
        ...
        fg_mask1_new_coors = nn_1[fg_mask1]
        fg_mask1_mask_new_coors = torch.zeros(num_patches2[0] * num_patches2[1], dtype=torch.bool, device=self.device)
        fg_mask1_mask_new_coors[fg_mask1_new_coors] = True
        bbs_mask = torch.bitwise_and(fg_mask2, fg_mask1_mask_new_coors)
        indices_to_show = torch.nonzero(bbs_mask, as_tuple=False).squeeze(dim=1)
        img2_indices_to_show = torch.arange(num_patches2[0] * num_patches2[1], device=self.device)[indices_to_show]
        img1_indices_to_show = nn_2[indices_to_show]
        ...
        if do_postprocess:
            ...
            return ParserNet._postprocess(results, batched_inputs, images.image_sizes, max_shape)
        else:
            return results

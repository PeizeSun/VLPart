...
class GeneralizedRCNNX(nn.Module):
    @configurable
    def __init__(
        self,
        *,
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if self.roi_head_name in ['StandardROIHeads', 'CascadeROIHeads', 'OLNROIHeads']:
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return GeneralizedRCNNX._postprocess(results, batched_inputs, images.image_sizes, max_shape)
        else:
            return results
...
def custom_detector_postprocess(
    ...
    if results.has("pred_global_masks"):
        mask_pred_per_image = F.interpolate(results.pred_global_masks.unsqueeze(1), size=max_shape, mode="bilinear", align_corners=False)
        mask_pred_per_image = mask_pred_per_image[:,:, :resized_h, :resized_w]
        mask_pred_per_image = F.interpolate(mask_pred_per_image, size=new_size, mode='bilinear', align_corners=False).squeeze(1)

...
class BoxFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.box_score_loss_weight = box_score_loss_weight
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'box_score_loss_weight': cfg.MODEL.ROI_BOX_HEAD.BOX_SCORE_LOSS_WEIGHT,
        })
        return ret
    ...
    def losses(self, predictions, proposals):
        ...
        loss_dict = {
            "loss_box_score": loss_box_score * self.box_score_loss_weight,
...
class ClassificationFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.obj2part_mapping = load_obj2part_mapping(obj2part_mapping)
        self.part2proxy_mapping = load_obj2part_mapping(part2proxy_mapping)
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'obj2part_mapping': cfg.MODEL.ROI_BOX_HEAD.OBJ2PART_MAPPING,
            'part2proxy_mapping': cfg.MODEL.ROI_BOX_HEAD.PART2PROXY_MAPPING,
        })
    ...
    def classification_losses(self, predictions, proposals):
        ...
        loss_dict = {
            "loss_cls": loss_cls,
            # "loss_part": scores.new_zeros([1])[0],
    ...
    def classification_part_losses(self, predictions, proposals):
        scores = predictions
        gt_classes = (
            cat([p.gt_classes for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        )
    ...
    def box2part_losses(self, part_predictions, proposals, targets, classifier_info=(None, None, None), ):
        ...
        for idx, (part_score, proposal, target) in enumerate(zip(part_scores, proposals, targets)):
            ...
            for obj_box, obj_cat in zip(gt_boxes, gt_classes):
                ...
                loss_part = self._box2part_loss(part_score, part_cat_idx, obj_box, proposal)
                part_loss += loss_part
                part_num += len(part_cat_idx)

        part_loss = part_loss / max(part_num, 1)
        loss_dict = {
    ...
    def _box2part_loss(self, score, part_target_ind, obj_box, proposal):
        fg_iou = pairwise_ioa(Boxes(obj_box.reshape(-1,4)), proposal.proposal_boxes)
        inside_score = score * (fg_iou > 0.9).float().transpose(0, 1)
        loss = score[0].new_zeros([1])[0]
        for part_id in part_target_ind:
            with torch.no_grad():
                proxy_ind = self.part2proxy_mapping[str(part_id)]
                proxy_score = inside_score[:, proxy_ind]
                part_pro_ind = torch.div(torch.argmax(proxy_score.reshape(-1)), len(proxy_ind), rounding_mode='trunc')
                target = score.new_zeros(score.shape[1])
                target[part_id] = 1.
            loss += F.binary_cross_entropy_with_logits(
                score[part_pro_ind], target, reduction='sum')
        return loss

    def img2part_losses(self, part_predictions, proposals, targets, classifier_info=(None, None, None), ):
        num_inst_per_image = [len(p) for p in proposals]
        part_scores = part_predictions.split(num_inst_per_image, dim=0)
        ...
        for idx, (part_score, proposal, target) in enumerate(zip(part_scores, proposals, targets)):
            if part_score.shape[0] == 0:
                continue
            obj_cat = 11
            part_cat_idx = self.obj2part_mapping[str(obj_cat)]
            loss_part = self._img2part_loss(part_score, part_cat_idx)
            part_loss += loss_part
            part_num += len(part_cat_idx)
    ...
    def _img2part_loss(self, score, part_target_ind, obj_box=None, proposal=None):
        inside_score = score
        loss = score[0].new_zeros([1])[0]
        for part_id in part_target_ind:
            with torch.no_grad():
    ...
    def _max_nearest_score_loss(self, score, label, p):
        cat2dog = {31: 49, 32: 50, 33: 51, 34: 52, 35: 53, 36: 54, 37: 55, 38: 57, 39: 58}
        dog2cat = {value-1: key-1 for key, value in cat2dog.items()}

        target = score.new_zeros(score.shape[1])
        target[label] = 1.
        label = label.item()
        nearest_label = dog2cat[label] if label in dog2cat else label
        nearest_label = label
        ind = score[:, nearest_label].argmax().item()

        cat_score = score[ind, nearest_label].sigmoid()
        # cat_name = CATEGORIES[label]['name']
        cat_name = CATEGORIES[nearest_label]['name']
        import cv2
        image_show = cv2.imread('show.png')
        bbox = p.proposal_boxes.tensor[ind]
        image_show = cv2.rectangle(
            image_show, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])),
            (255, 0, 0), 2)
        image_show = cv2.putText(image_show, "{}:{:.2f}".format(cat_name, cat_score),
                                 (int(bbox[0]), int(bbox[1])), cv2.FONT_HERSHEY_SIMPLEX,
                                 1.0, (0, 0, 255), 2)
        cv2.imwrite('show.png', image_show)
        loss = F.binary_cross_entropy_with_logits(

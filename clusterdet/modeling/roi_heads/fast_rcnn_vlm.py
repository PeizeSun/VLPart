...
from clusterdet.data.datasets.pascal_part import PASCAL_PART_CATEGORIES as CATEGORIES
...
class VLMFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def __init__(
        ...
        *,
        ...
        part_cls_cost_weight=2,
        part_loc_cost_weight=5,
        ...
        **kwargs,
        ...
        self.part_cls_cost_weight = part_cls_cost_weight
        self.part_loc_cost_weight = part_loc_cost_weight
    ...
    def from_config(cls, cfg, input_shape):
        ...
        ret.update({
            ...
            'part_cls_cost_weight': cfg.MODEL.ROI_BOX_HEAD.PART_CLS_COST_WEIGHT,
            'part_loc_cost_weight': cfg.MODEL.ROI_BOX_HEAD.PART_LOC_COST_WEIGHT,
    ...
    def predict_probs(self, predictions, proposals):
        ...
        for cls_score in cls_scores:
            ...
            # mask = torch.zeros_like(final_score)[0]
            # mask[torch.arange(30,39)] = 1.
            # final_score = final_score * mask[None]
    ...
    def part_classification_losses(self, predictions, proposals, targets):
        ...
        loss_dict = {
            ...
            "loss_ppart": loss_cls * self.box2part_loss_weight,
            'loss_noun': scores.new_zeros([1])[0],
            # 'loss_caption': scores.new_zeros([1])[0],
        }
        return loss_dict
    ...
    def part_classification_losses_x(self, predictions, proposals, targets):
        ...
        for idx, (cls_score, proposal, target) in enumerate(zip(cls_scores, proposals, targets)):
            ...
            for idx, label in enumerate(pos_category_ids):
                if self.part_loss_type == 'max_size':
                    ...
                    loss_i = self._max_loc_score_loss(
                        cls_score, label, pos_boxes[idx], proposal)
        ...
        loss_dict = {
            ...
            # 'loss_caption': cls_scores[0].new_zeros([1])[0],
    ...
    def _image_loss(self, score, label, p):
    ...
    def _max_loc_score_loss(self, score, label, box, proposal):
        target = score.new_zeros(score.shape[1])
        target[label] = 1.
        cost_loc = -(1. - giou_loss(proposal.proposal_boxes.tensor, box.tensor))
        prob = score[:, label].sigmoid()
        cost_cls = -(prob + 1e-8).log() - (-(1 - prob + 1e-8).log())
        cost = self.part_loc_cost_weight * cost_loc + \
               self.part_cls_cost_weight * cost_cls
        ind = cost.argmin().item()
        loss = F.binary_cross_entropy_with_logits(
            score[ind], target, reduction='sum')
        return loss

...
from clusterdet.data import (
    DatasetMapperAnn,
    DatasetMapperWithImage,
    DatasetMapperFilterByBox,
    DatasetMapperCaptionParser,
)
from clusterdet.data.custom_build_augmentation import build_custom_augmentation
from clusterdet.data.custom_dataset_dataloader import (
    build_custom_train_loader,
    build_custom_test_loader,
)
from clusterdet.solver.custom_solver import build_custom_optimizer
from clusterdet.evaluation import (
    COCOUnsupEvaluator,
...
from clusterdet import add_clusterdet_config
import warnings
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type in ["coco"]:
        if cfg.MODEL.SUPERVISION == 'unsupervised':
            evaluator_list.append(COCOUnsupEvaluator(dataset_name, output_dir=output_folder))
        elif cfg.MODEL.SUPERVISION == 'open_world':
            evaluator_list.append(COCOOpenWorldEvaluator(cfg, dataset_name, output_dir=output_folder))           
        elif cfg.MODEL.SUPERVISION == 'open_vocabulary':
            evaluator_list.append(COCOOpenVocabularyEvaluator(dataset_name, output_dir=output_folder))            
        else:
            evaluator_list.append(COCOEvaluator(dataset_name, output_dir=output_folder))
    if evaluator_type == "o365":
        return O365Evaluator(dataset_name, output_dir=output_folder)
...
def do_test(cfg, model):
    ...
    mapper = DatasetMapperAnn(cfg, is_train=False) \
        if cfg.MODEL.ANN_GENERATOR or cfg.MODEL.CLASS_FILTER else None
...
def do_train(cfg, model, resume=False):
    ...
    # if cfg.DATALOADER.CAPTION_PARSER:
    #     MapperClass = DatasetMapperCaptionParser
    # else:
